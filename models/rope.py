...
class _RotateFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: Tensor):
        sin, cos = nn_utils.saved_tensors(ctx, grad_output.dtype)
...
class RoPE(nn.Module):
    def __init__(self, base: float | int = None):
        ...
        if base is None:
            base = 10000
        dev_utils.type_check(("base", base, float | int), func_name="RoPE.__init__()")
        if base <= 0:
    ...
    @staticmethod
    def compute_sin_cos(
        T: int, D: int, device: torch.device, dtype: torch.dtype, base: int | float
    ) -> tuple[Tensor, Tensor]:
        angles = (
    ...
    def base(self, x: int | float):
        dev_utils.type_check(("x", x, int | float), func_name="RoPE.base.setter()")
        self._base = x

        if hasattr(self, "cached_sin"):
            self.__delattr__("cached_sin")
        if hasattr(self, "cached_cos"):
