...
class _LinearFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: Tensor) -> tuple[Tensor, Tensor, Tensor | None]:
        x, weight = nn_utils.saved_tensors(ctx, grad_output.dtype)
...
class Linear(nn.Module):
    def __init__(
        ...
        *,
        ...
        if in_features <= 0:
            raise ValueError(f"<{func_name}> in_features는 양의 정수여야 합니다.")
        if out_features <= 0:
            raise ValueError(f"<{func_name}> out_features는 양의 정수여야 합니다.")
    ...
    def forward_debug(self, x: Tensor):
        func_name = "Linear.forward()"
        if x.size(-1) != self.in_features:
            raise ValueError(
                f"<{func_name}> 입력 텐서의 마지막 차원의 크기가 부적절합니다. "
    ...
    @property
    def bias(self):
        return self._bias if self.use_bias else None
