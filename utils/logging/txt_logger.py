        ...
        with self._path.open(mode='a', encoding='utf-8') as f:
            ...
            match event:
                ...
                case TrainingEvent.TRAIN_STARTED:
                    record += (
                        f"event=TRAIN_STARTED"
                    )
                case TrainingEvent.TRAIN_STOPPED:
                    record += (
                        f"event=TRAIN_STOPPED, step= {trainer.current_step}, reason= user_request"
                    )
                case TrainingEvent.TRAIN_RESUMED:
                    record += (
                        f"event=TRAIN_RESUMED, step= {trainer.current_step}, reason= user_request"
                    )
                case TrainingEvent.TRAIN_COMPLETED:
                    match reason:
                        case TrainLoopResult.USER_CANCELLED:
                            record += (
                                f"event=TRAIN_COMPLETED, step= {trainer.current_step}, reason= user_request"
                            )
                        case TrainLoopResult.MAX_STEPS_REACHED:
                            record += (
                                f"event=TRAIN_COMPLETED, step= {trainer.current_step}, reason= max_steps_reached"
                            )
                case TrainingEvent.EPOCH_COMPLETED:
                    record += (
                        f"event=EPOCH_COMPLETED, step= {trainer.current_step}, reason= epoch_completed"
                    )
