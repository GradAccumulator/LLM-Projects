...
class Tokenizer:
    def __init__(self, model_name: str, num_threads: int = os.cpu_count() // 2):
        self._processor = spm.SentencePieceProcessor(
            model_file=rf"tokenizer\models\{model_name}\{model_name}.model"
        )
    ...
    def encode(self, x, return_type=None):
        if issubclass(return_type, np.generic):
            return np.asarray(
                self._processor.encode(x, num_threads=self.num_threads),
                dtype=return_type,
            )
        return self._processor.encode(
            x, return_type=return_type, num_threads=self.num_threads
        )
